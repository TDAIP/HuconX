...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RayfieldLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show HuconX'.", Duration = 7, Image = 4400697855})
		else
...
function RayfieldLibrary:CreateWindow(Settings)
    ...
    -- Clone the existing toggle template
    ...
    -- Nếu có "Plus", hiển thị nhãn Plus
    if ToggleSettings.Plus then
        local PlusLabel = Instance.new("TextLabel")
        PlusLabel.Text = "Plus"
        PlusLabel.Size = UDim2.new(0, 50, 0, 20)  -- Có thể điều chỉnh kích thước và vị trí theo ý muốn
        PlusLabel.Position = UDim2.new(1, 5, 0, 0)  -- Đặt nhãn "Plus" ở phía bên phải toggle
        PlusLabel.Parent = Toggle
    end
